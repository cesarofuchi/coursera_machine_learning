...
%% Part 1
% adicionar 1 para igualar matriz (bias e features)
% 5000x400<- 401x25
% 5000x401
X=[ones(m,1),X];
%%
%calcular parâmetros a e z
a1=X; %5000x401
z2=a1*Theta1'; %5000x401 401x25
a2=sigmoid(z2); 
a2=[ones(size(a2,1),1),a2]; %25 -> 26
z3=a2*Theta2';  %5000x26 26x10
a3=sigmoid(z3); %5000x10 
h_x=a3;

%% conveter y de 1-10 para 1 e 0
...
%% Part2 calcular o gradiente com backpropagation!

%primeiro erro A3 pois são 3 camadas

delta3=a3-yv; %5000x10 - 5000x10
% %vetor de bias 5000x1
bias2=ones(size(z2,1),1); 
% % 5000x10  10x26 = 5000x26 .* 5000x(25+1)
%cuidado com essa linha
delta2=(delta3*Theta2).*[ones(size(z2,1),1) sigmoidGradient(z2)];
% %voltando a 5000x25
delta2=delta2(:,2:end); % remover primeira linha
  


%% %%% gradiente acumulado vetorizado
